...
#Training params
trainID="UNet2D_B0toTensor_L1Loss"
num_epochs = 1000
lr = 1e-4
...
if __name__ == "__main__" :
    ...
    # model = UNet(in_channels=n_channels, out_channels=out_channels, depth=depth, wf=wf, padding=padding,
    #             batch_norm=batch_norm, up_mode=up_mode, droprate=dropout_prob, is3D=is3D, 
    #             returnBlocks=False, downPath=True, upPath=True)
    # model = ResNet(in_channels=n_channels, out_channels=out_channels)
    model = UNet(in_channels=n_channels, n_classes=out_channels, depth=depth, wf=wf, padding=True)
    model.to(device)
